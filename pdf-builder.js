...
(function () {
    ...
    function buildTheory() {
        ...
        return `<h1 class="chapter">📘 1. Teorie — Cele 6 prepoziții fixe Akkusativ (ULTIMA A1 🏁)</h1>` + t;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillInBlock('Übung 1 — Cele 6 prepoziții care cer MEREU Akkusativ', '🎯 BIS · DURCH · FÜR · GEGEN · OHNE · UM — toate cer Akkusativ! Memorează în lant: „bis durch für gegen ohne um".', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += fillInBlock('Übung 2 — für + Akkusativ + Possessiv (pentru cine?)', 'für + Akk: für meinen Bruder (m) · für meine Mutter (f) · für mein Kind (n) · für meine Eltern (pl). Posesivele primesc -en la Akk masc.', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += fillInBlock('Übung 3 — „um ... herum" (în jurul) — construcție specială', '„um X herum" = în jurul lui X. „herum" stă la sfârșit. Ex.: „Der Hund läuft um den Tisch herum." (Câinele aleargă în jurul mesei.)', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillInBlock('Übung 4 — Personalpronomen Akk recap (după prepoziții fixe)', 'Pronumele Akk recap din L12: mich/dich/ihn/sie/es/uns/euch/sie/Sie. „für ihn" (pentru el) · „ohne sie" (fără ea/ele) · „gegen uns" (împotriva noastră).', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += fillInBlock('Übung 5 — Mix toate cele 6 prepoziții în context', 'Pattern: SUBIECT + VERB + [prepoziție Akk] + obiect. Atenție la gen + Akkusativ + posesiv/articol corect.', ex5Data);
        }
        return html;
    }
